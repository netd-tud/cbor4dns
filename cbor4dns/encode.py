...
class RR(HasTypeSpec):
    ...
    def __init__(
        ...
        self.name = self.ref_idx.add(name)
        self.ttl = ttl
        if rdata.rdtype in TEXT_STRING_TYPES:
            self.rdata = self.ref_idx.add(
                rdata.to_text(omit_final_dot=True).strip(".")
            )
        else:
            self.rdata = rdata
    ...
    def rrs_from_rrset(cls, rrset, question: Question, ref_idx: RefIdx) -> list:
        if isinstance(rrset, dns.rrset.RRset):
            ...
            return [
                cls(
                    rrset.name.to_text(omit_final_dot=True),
...
class Encoder:
    ...
    def _get_question(self, msg):
        ...
        return Question(
            question_section.name.to_text(omit_final_dot=True),
